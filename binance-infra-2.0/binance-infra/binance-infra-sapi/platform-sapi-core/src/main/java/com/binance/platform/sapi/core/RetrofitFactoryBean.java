...
import com.binance.platform.sapi.annotation.Intercept;
import com.binance.platform.sapi.annotation.InterceptMark;
import com.binance.platform.sapi.annotation.Intercepts;
import com.binance.platform.sapi.annotation.OkHttpClientBuilder;
import com.binance.platform.sapi.annotation.Sapi;
import com.binance.platform.sapi.degrade.BaseResourceNameParser;
import com.binance.platform.sapi.degrade.Degrade;
import com.binance.platform.sapi.degrade.DegradeType;
import com.binance.platform.sapi.degrade.FallbackFactory;
import com.binance.platform.sapi.degrade.RetrofitDegradeRule;
import com.binance.platform.sapi.degrade.RetrofitDegradeRuleInitializer;
import com.binance.platform.sapi.degrade.SentinelDegradeInterceptor;
import com.binance.platform.sapi.interceptor.BaseGlobalInterceptor;
import com.binance.platform.sapi.interceptor.BaseLoggingInterceptor;
import com.binance.platform.sapi.interceptor.BasePathMatchInterceptor;
import com.binance.platform.sapi.interceptor.ErrorDecoderInterceptor;
import com.binance.platform.sapi.interceptor.LogLevel;
import com.binance.platform.sapi.interceptor.LogStrategy;
import com.binance.platform.sapi.interceptor.NetworkInterceptor;
import com.binance.platform.sapi.interceptor.ServiceInstanceChooserInterceptor;
import com.binance.platform.sapi.degrade.DegradeStrategy;
import com.binance.platform.sapi.config.DegradeProperty;
import com.binance.platform.sapi.config.LogProperty;
import com.binance.platform.sapi.config.RetrofitConfigBean;
import com.binance.platform.sapi.config.RetrofitProperties;
import com.binance.platform.sapi.util.ApplicationContextUtils;
import com.binance.platform.sapi.util.BeanExtendUtils;
...
public class RetrofitFactoryBean<T> implements FactoryBean<T>, EnvironmentAware, ApplicationContextAware {
    ...
    public RetrofitFactoryBean(Class<T> retrofitInterface) {
        ...
        sapi = retrofitInterface.getAnnotation(Sapi.class);
    }
    ...
    public T getObject() throws Exception {
        ...
        Class<?> fallbackClass = sapi.fallback();
        ...
        Class<?> fallbackFactoryClass = sapi.fallbackFactory();
    ...
    private void checkRetrofitInterface(Class<T> retrofitInterface) {
        ...
        Assert.isTrue(StringUtils.hasText(sapi.baseUrl()) || StringUtils.hasText(sapi.serviceId()),
    ...
    private synchronized ConnectionPool getConnectionPool(Class<?> retrofitClientInterfaceClass) {
        Sapi sapi = retrofitClientInterfaceClass.getAnnotation(Sapi.class);
        String poolName = sapi.poolName();
    ...
    private synchronized OkHttpClient getOkHttpClient(Class<?> retrofitClientInterfaceClass)
        ...
        if (method != null) {
            ...
            ConnectionPool connectionPool = getConnectionPool(retrofitClientInterfaceClass);

            final int connectTimeoutMs = sapi.connectTimeoutMs() == -1 ? retrofitProperties.getGlobalConnectTimeoutMs() : sapi.connectTimeoutMs();
            final int readTimeoutMs = sapi.readTimeoutMs() == -1 ? retrofitProperties.getGlobalReadTimeoutMs() : sapi.readTimeoutMs();
            final int writeTimeoutMs = sapi.writeTimeoutMs() == -1 ? retrofitProperties.getGlobalWriteTimeoutMs() : sapi.writeTimeoutMs();
            final int callTimeoutMs = sapi.callTimeoutMs() == -1 ? retrofitProperties.getGlobalCallTimeoutMs() : sapi.callTimeoutMs();
            ...
            okHttpClientBuilder = new OkHttpClient.Builder()
                    ...
                    .retryOnConnectionFailure(sapi.retryOnConnectionFailure())
                    .followRedirects(sapi.followRedirects())
                    .followSslRedirects(sapi.followSslRedirects())
                    .pingInterval(sapi.pingIntervalMs(), TimeUnit.MILLISECONDS)
        ...
        if (StringUtils.hasText(sapi.serviceId())) {
        ...
        Class<? extends ErrorDecoder> errorDecoderClass = sapi.errorDecoder();
        ...
        if (logProperty.isEnable() && sapi.enableLog()) {
            ...
            LogLevel logLevel = sapi.logLevel();
            LogStrategy logStrategy = sapi.logStrategy();
    ...
    private synchronized Retrofit getRetrofit(Class<?> retrofitClientInterfaceClass) throws InstantiationException, IllegalAccessException, NoSuchMethodException, InvocationTargetException {
        ...
        Retrofit.Builder retrofitBuilder = new Retrofit.Builder()
                .baseUrl(baseUrl)
                .validateEagerly(sapi.validateEagerly())
                .client(client);

        // 添加CallAdapter.Factory
        Class<? extends CallAdapter.Factory>[] callAdapterFactoryClasses = sapi.callAdapterFactories();
        ...
        // 添加Converter.Factory
        Class<? extends Converter.Factory>[] converterFactoryClasses = sapi.converterFactories();

...
import static com.binance.platform.EurekaConstants.EUREKA_METADATA_SERVERLOAD;
...
public class DefaultGrayServerFilter implements ServerFilter {
    ...
    // AB测试放百分之10的流量
    private static final String DEFAULT_AB_TEST_PERCENT = "5";
    ...
    protected void doMatchEnvKey(final Set<Server> filteredServer, final DiscoveryEnabledServer server) {
        ...
        // 如果有路由规则，则匹配
        if (envKey != null) {
            boolean envKeyEqual = StringUtils.equalsIgnoreCase(envKeyMeta, envKey);
            if (envKeyEqual) {
                filteredServer.add(server);
            }
            ...
            if (StringUtils.equalsIgnoreCase(EUREKA_META_ENVKEY_DEFAULT, envKeyMeta)) {
                ...
                // 如果是生产，如果节点上打了灰度标签，且打了标签说需要放量，默认放百分之10的流量
                // 注释原因： 在蓝绿部署的情况下，灰度节点不放请求进来
                // 如果是生产，如果节点上打了灰度标签，且打了标签说需要放量，默认放百分之10的流量
                // 注释原因： 在蓝绿部署的情况下，灰度节点不放请求进来
                // if (EnvUtil.isProd()
                // && StringUtils.equalsIgnoreCase(EurekaConstants.EUREKA_META_ENVKEY_GRAY, envKeyMeta)) {
                // boolean grayflow = BooleanUtils
                // .toBoolean(instanceMeta.getOrDefault(EUREKA_METADATA_GRAYFLOW, "false"));
                // if (grayflow) {
                // int randomValue = LEAST_FLOW_RANDOM.nextInt(100);
                // if (randomValue <= LEAST_FLOW_PERCENT) {
                // filteredServer.add(server);
                ...
                // }
                // }
                boolean grayflow = BooleanUtils.toBoolean(instanceMeta.getOrDefault(EUREKA_METADATA_GRAYFLOW, "false"));
                if (grayflow) {
                    int randomValue = LEAST_FLOW_RANDOM.nextInt(100);
                    if (randomValue <= LEAST_FLOW_PERCENT) {
                        filteredServer.add(server);
    ...
    // 计算tomcat的负载情况
    protected void doTomcatLoad(Set<Server> filteredServer) {
        if (filteredServer.size() > 2) {
            Iterator<Server> it = filteredServer.iterator();
            while (it.hasNext()) {
                Server server = it.next();
                if (server instanceof DiscoveryEnabledServer) {
                    DiscoveryEnabledServer discoveryEnabledServer = (DiscoveryEnabledServer) server;
                    Map<String, String> eurekaMetadata = discoveryEnabledServer.getInstanceInfo().getMetadata();
                    String serverLoadJson = eurekaMetadata.get(EUREKA_METADATA_SERVERLOAD);
                    if (StringUtils.isNotBlank(serverLoadJson)) {
                        ...
                        if (serverLoadStatus != null) {
                            int busyThreshold = Double.valueOf(serverLoadStatus.getMaxThreads() * 0.25).intValue();
                            if (busyThreshold > 0 && serverLoadStatus.getCurrentThreadsBusy() > busyThreshold) {
                                it.remove();
                                LOGGER.info("server:{} has been remove,because of tomcat busy thread equal max thread's 25%",
                                    server.getHostPort());
                            }
                        }
                    }
    ...
    public List<Server> match(List<Server> servers) {
        ...
        this.LEAST_FLOW_PERCENT = Integer
                .valueOf(environment.getProperty("com.binance.leastflow.percent", DEFAULT_AB_TEST_PERCENT));
        ...
        if (warmUpServer.size() > 2) {
            ...
            while (it.hasNext()) {
                ...
                if (server instanceof DiscoveryEnabledServer) {
                    DiscoveryEnabledServer discoveryEnabledServer = (DiscoveryEnabledServer) server;
                    Map<String, String> eurekaMetadata = discoveryEnabledServer.getInstanceInfo().getMetadata();
                    boolean warmUp = BooleanUtils
                            .toBoolean(eurekaMetadata.getOrDefault(EUREKA_METADATA_WARMUP, "true"));
                    if (warmUp) {
                        // 如果服务在eureka上的注册时间少于1分钟，然后取一下随机数，如果随机数大于10，则把该节点从列表里删除
                        long warmUpDelayTime = Long
                                .valueOf(eurekaMetadata.getOrDefault(EUREKA_METADATA_WARMUP_DELAY_TIME, "120000"));
                        if (discoveryEnabledServer.getInstanceInfo().getLeaseInfo()
                                .getServiceUpTimestamp() <= warmUpDelayTime) {
                            int randomValue = LEAST_FLOW_RANDOM.nextInt(100);
                            if (randomValue >= LEAST_FLOW_PERCENT) {
                                it.remove();
                                LOGGER.info("server:{} has been remove,because of warm up.", server.getHostPort());
                            }
                        }
                    }
                }
